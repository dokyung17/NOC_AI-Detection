...
rPPG (pyVHR slim) + D3 temporal + Laplacian spatial
+ blending-boundary + ArcFace identity consistency → single CSV per run.
"""
...
from features.face_utils import FaceMeshDetector, extract_boundary_and_identity_features  # noqa: E402
from features.identity import load_arcface_model  # noqa: E402
...
def process_video(
    ...
    try:
        ...
        row.update(
            extract_boundary_and_identity_features(
                video_path,
                rec_model=rec_model,
...
def _init_worker(
    ...
    arcface_dir: Optional[str],
) -> None:
    """Load Pipeline + D3 + FaceMesh + ArcFace once per worker process."""
    os.environ.setdefault("KMP_DUPLICATE_LIB_OK", "TRUE")
    ...
    _WORKER["face_detector"] = FaceMeshDetector()
    _WORKER["rec_model"] = load_arcface_model(
        device,
        model_dir=Path(arcface_dir) if arcface_dir else None,
    )
...
def _worker_process_video(
    ...
    row = process_video(
        ...
        face_detector=_WORKER["face_detector"],  # type: ignore[arg-type]
        rec_model=_WORKER["rec_model"],
    )
    return idx, row
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Unified rPPG + D3 + boundary + identity feature extraction"
    )
    parser.add_argument(
        "inputs",
        nargs="*",
    ...
    parser.add_argument(
        "--arcface-dir",
        default=None,
        help="Directory containing w600k_r50.onnx (default: models/arcface)",
    )
    ...
    if device.type == "cuda":
        ...
        print("rPPG cpu_POS will continue to run on CPU.")
    print("Boundary/identity use MediaPipe FaceMesh even when --no-face is set.")
    ...
    if args.workers == 1:
        ...
        model.eval()
        rec_model = load_arcface_model(
            device,
            model_dir=Path(args.arcface_dir) if args.arcface_dir else None,
        )
