...
def _make_model(num_class, random_state=42):
    ...
    if num_class == 2:
        # 클래스가 2개(예: real vs diffusion만 남은 --exclude-deepfake)면 이진 목적함수로 전환.
        # multi:softprob을 num_class=2로 그대로 쓰면 XGBoost가 .predict()에서
        # 클래스 라벨이 아닌 다른 형태를 반환해 오류가 생길 수 있다.
        params.update(objective="binary:logistic", eval_metric="logloss")
    else:
        params.update(objective="multi:softprob", num_class=num_class, eval_metric="mlogloss")
...
def run_leave_one_subgroup_out(df: pd.DataFrame, class_names, random_state: int = 42) -> pd.DataFrame:
    """
    ...
    for held_out in subgroups:
        ...
        model = _make_model(num_class=len(class_names), random_state=random_state)
...
def main():
    ...
    parser.add_argument("--out-csv", default="subgroup_generalization_results.csv",
                         help="결과를 저장할 CSV 경로")
    parser.add_argument("--exclude-deepfake", action="store_true",
                         help="deepfake 관련 서브그룹(faceforensics(__), id_id(faceswap))을 제외하고 "
                              "real vs diffusion 스코프로만 평가")
    ...
    if args.exclude_deepfake:
        before_n = len(df)
        df = df[df["group"] != "deepfake"].reset_index(drop=True)
        print(f"[디퓨전 전용 스코프] deepfake 서브그룹 제외 -> {before_n}개 -> {len(df)}개")
    ...
    result = run_leave_one_subgroup_out(df, class_names=sorted(df["group"].unique().tolist()))
