...
def main():
    ...
    parser.add_argument("--diffusion-only", action="store_true",
                         help="deepfake 행을 아예 제외하고 real vs diffusion 2-class로만 학습/평가 "
                              "(--binary, --hierarchical과 동시 사용 불가)")
    ...
    if args.diffusion_only:
        if args.binary or args.hierarchical:
            raise SystemExit("--diffusion-only은 --binary, --hierarchical과 동시에 쓸 수 없어요.")
        before_n = len(df)
        df = df[df["group"] != "deepfake"].reset_index(drop=True)
        print(f"[디퓨전 전용 스코프] deepfake {before_n - len(df)}개 제외 -> {len(df)}개 (real + diffusion만)")
    ...
    if args.weight_search:
        ...
        y = df["group"].values
        class_names = ["real", "diffusion"] if args.diffusion_only else ["real", "diffusion", "deepfake"]
    ...
    if args.binary:
        ...
        class_names = ["real", "diffusion"]
        default_model_out = "xgb_model_realvsdiffusion.joblib"
    else:
    ...
    mode_label = "이진(real vs fake)" if args.binary else ("real vs diffusion" if args.diffusion_only else "3진")
    print(f"\n=== 5-fold 교차검증 평가 ({mode_label} 분류) ===")
